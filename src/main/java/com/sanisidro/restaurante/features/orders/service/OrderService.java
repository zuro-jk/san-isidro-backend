...
public class OrderService {
    ...
    public OrderResponse create(OrderRequest request, String lang) {
        Order order = buildOrderBase(request);
    ...
    public OrderResponse update(Long id, OrderRequest request, String lang) {
        ...
        order.setEmployee(request.getEmployeeId() != null ?
                ...
                : null);
        order.setAddress(request.getAddressId() != null ?
        ...
        order.getDetails().forEach(detail ->
                applyInventoryMovement(detail.getProduct(), detail.getQuantity(), true, order.getId(), "Actualización de orden")
        );
        ...
        for (OrderDetailInOrderRequest d : request.getDetails()) {
            ...
            applyInventoryMovement(product, d.getQuantity(), false, order.getId(), "Actualización de orden");
    ...
    public void delete(Long id) {
        ...
        order.getDetails().forEach(detail ->
                applyInventoryMovement(detail.getProduct(), detail.getQuantity(), true, order.getId(), "Eliminación de orden")
        );
    ...
    private void applyInventoryMovement(Product product, Integer quantity, boolean restore, Long referenceId, String reason) {
        product.getIngredients().forEach(pi -> {
            Inventory inventory = inventoryRepository.findByIngredient(pi.getIngredient())
                    .orElseThrow(() -> new InventoryNotFoundException("Inventario no encontrado para ingrediente: " + pi.getIngredient().getName()));
            BigDecimal totalQty = BigDecimal.valueOf(pi.getQuantity()).multiply(BigDecimal.valueOf(quantity));

            if (restore) inventory.increaseStock(totalQty);
            else inventory.decreaseStock(totalQty);
        ...
        for (User admin : admins) {
            try {
                StockLowNotificationEvent event = StockLowNotificationEvent.builder()
                        ...
                        .message("El stock del ingrediente '" + inventory.getIngredient().getName() +
                                "' ha bajado a " + inventory.getCurrentStock() +
                                " unidades. Stock mínimo: " + inventory.getMinimumStock())
                ...
                kafkaProducerService.sendMessage(
                        KafkaMessage.builder()
                                .topic("notifications")
                                .key("STOCK_LOW_" + inventory.getIngredient().getId() + "_" + admin.getId())
                                .payload(payload)
                ...
                log.error("❌ Error enviando notificación de stock bajo para admin {}", admin.getEmail(), e);
            }
        }
    }
